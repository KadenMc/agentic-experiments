...
exec aexp queue run --tag overnight --index "$SLURM_ARRAY_TASK_ID"
```
...
Submit with `sbatch overnight.sbatch`. Supply `#SBATCH` directives via
...
git pull
# overnight.sbatch is your site-tuned script, ending in:
#   aexp queue run --tag overnight --index "$SLURM_ARRAY_TASK_ID"
...
writing `overnight.sbatch` from scratch:

```
aexp queue materialize --tag overnight --runner slurm --output overnight.sbatch
# Edit overnight.sbatch to fill in #SBATCH directives + site setup.
```
